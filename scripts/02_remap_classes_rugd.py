...
input_root = Path("../data/processed/rugd/annotations_with_id")
output_root = Path("../data/processed/combined/rugd/annotations_with_id")
...
def load_and_convert(df):
    ...
    for i, mask in enumerate(df["path"]):
...
def write_2_file(old_path, scene, new_mask):
    file_path = Path(scene) / Path(old_path.name)
